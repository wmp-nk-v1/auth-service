...
eks-deploy:
	...
	helm upgrade -i auth-service helm -f helm/values/auth-service.yml --set image.tag=${image_tag}	

argocd-deploy:
	argocd login $(argocd_server) --insecure --username admin --password $(argocd_admin_password)
	argocd app create auth-service --sync-policy auto --repo https://github.com/nikkaushal/wmp-helm-v1.git --path . --dest-server https://kubernetes.default.svc   --dest-namespace default --helm-set-string image.tag=$(image_tag) --values values/auth-service.yml --upsert